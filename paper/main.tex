...
\author{Sankalp Pathak\\ pathaksankalp04@gmail.com \\ Jaypee University of Engineering and Technology Guna \and Sanjay Garg \\ gargsv@gmail.com \\ Jaypee University of Engineering and Technology Guna}
...
as general visual backbones. We present the first study of PEFT
...
breaks head-compatibility. Whereas re-fitting a probe on supervised-ViT
merges only \emph{partially} restores accuracy~\citep{sharma2024nonlocal},
here recovery is essentially complete, localizing interference almost
entirely to the head. Second, a rate--distortion-optimal merging encoder
...
(1) The first study of PEFT adapter merging on JEPA-family encoders.
(2) A \emph{sharpened} interference-localization result. Prior work on
supervised-ViT merging used the same probe-refit instrument and found that
re-fitting only \emph{partially} recovers merged-model
accuracy~\citep{sharma2024nonlocal}; we show that on JEPA-family SSL backbones
at $k{=}2$ the re-fit probe recovers to ceiling (P1 mean $1.04$) while
original-head accuracy (P2) drops $15$--$24$ points---interference is
localized \emph{almost entirely} to head-compatibility, not partially. The
conceptual frame (probe re-fitting to localize interference) is not ours; the
completeness of the recovery on these backbones is the empirical
contribution. (3) Our principal methodological contribution: a
rate--distortion-optimal merging encoder (RD-encoder) that is the strongest
head-preserving merger across SSL families and two scales (smallest P1--P2
gap; best on P2 for four of five backbones; it ties on the well-merging
DINOv2). (4) An open release: metric library, adapters, and a SIGReg-based
isotropy-dial pretraining harness with a documented collapse-avoidance fix.
...
\paragraph{Closest prior work.} Two lines are directly adjacent.
\citet{kesim2024multilora} merge up to three LoRA adapters trained on six
supervised vision tasks and compare against a frozen-backbone,
fine-tuned-head baseline, reporting---as we do---that merging is achievable
with some loss of performance; their setting is supervised ViT rather than
JEPA-family SSL, and they do not separate feature- from head-level
interference. \citet{sharma2024nonlocal} ask precisely \emph{where} merging
interference originates: they re-fit a linear probe on the merged model to
test whether the penultimate-layer features remain compatible with the fixed
expert classifier, and propose rescaling and shifting the output activations
to compensate. This is the same probe-refit instrument we use, and the
head-localization frame is theirs, not ours; on supervised ViT they find that
probing only \emph{partially} restores merged accuracy. Our contribution
relative to them is empirical and sharper: on JEPA-family SSL backbones at
$k{=}2$ the re-fit probe recovers \emph{fully} (P1 mean $1.04$), localizing
interference almost entirely to the head, and we contribute a head-preserving
merger (RD-encoder) that closes the P1--P2 gap without any re-fitting.
...
To our knowledge no prior work studies adapter merging on JEPA-family
...
destruction of the underlying features. Where \citet{sharma2024nonlocal}
report that probe re-fitting only partially restores merged supervised-ViT
accuracy, here recovery is essentially complete (P1 mean $1.04$), indicating
that on these SSL backbones at low $k$ the features are left almost intact.
The practical reading is that on JEPA
...
\href{https://doi.org/10.5281/zenodo.20711952}{doi:10.5281/zenodo.20711952} (concept DOI, resolves to the latest version); current version \href{https://doi.org/10.5281/zenodo.20713190}{doi:10.5281/zenodo.20713190}; source at \url{https://github.com/K144U/jepa-adapter-merging}.
